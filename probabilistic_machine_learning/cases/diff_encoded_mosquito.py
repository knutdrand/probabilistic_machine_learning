...
def diff_encoded_model(diff_sampler, transition, init_state, observation_sampler, diff_prob, observation_prob):
    ...
    def sampler(T, key = jax.random.PRNGKey(0)):
        ...
        state = jax.lax.scan(sample_transition, state, jax.random.split(key, T - 1))[1]
        return observation_sampler(state)
    ...
    def log_prob(observed, logits_array, **P):
        ...
        return state_pdf + observation_prob(observed, state, P)
...
def refactor_model(param_dict):
    beta, lo_gamma, lo_a, lo_mu, logscale, reporting_rate = (param_dict[k] for k in ('beta', 'lo_gamma', 'lo_a', 'lo_mu', 'logscale', 'reporting_rate'))
    ...
    def observation_sampler(state):
        return s_stats.poisson(state[..., 2] * reporting_rate).rvs()
    ...
    return diff_encoded_model(diff_sampler, transition, jnp.array([0.9, 0.08, 0.01, 0.01]), observation_sampler, diff_prob, observation_prob)

if __name__ == '__main__':
    param_names = ['lo_gamma', 'lo_mu', 'lo_a', 'lo_mu', 'logscale', 'beta']
    real_params = {'beta': 0.3, 'lo_gamma': logit(0.1), 'lo_a': logit(0.1), 'lo_mu': logit(0.05), 'logscale': np.log(0.1), 'reporting_rate': 10000}
